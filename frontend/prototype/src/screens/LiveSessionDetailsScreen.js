...
export default function LiveSessionDetailsScreen({ route, navigation }) {
  ...
  return <View style={{ flex: 1, backgroundColor: colors.page }}><ScreenHeader title="Session Details" left={<IconButton icon="chevron-back" onPress={() => navigation.goBack()} />} /><ScrollView contentContainerStyle={{ padding: 18, gap: 14 }}>
    <Card style={{ gap: 8 }}><Text style={{ color: colors.ink, fontSize: 22, fontWeight: '900' }}>{session.sport} at {session.location}</Text><Text style={{ color: colors.text }}>{new Date(session.startsAt).toLocaleString()}</Text><Text style={{ color: colors.text }}>Coordinates: {session.locationPoint.coordinates.join(', ')}</Text><Text style={{ color: colors.text }}>{session.participants.length}/{session.maxParticipants} joined</Text><Text style={{ color: colors.text }}>Host: {session.host.name}</Text></Card>
